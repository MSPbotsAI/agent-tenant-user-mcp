...
RUN groupadd -g 1001 atum && \
    useradd -u 1001 -g atum -s /bin/sh -m atum
...
COPY --from=builder --chown=atum:atum /app/.venv /app/.venv
COPY --from=builder --chown=atum:atum /app/src /app/src

# Put venv on PATH so `python -m agent_tenant_user_mcp` resolves correctly
...
CMD ["python", "-m", "agent_tenant_user_mcp"]
